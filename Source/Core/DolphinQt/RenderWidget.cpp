...
void RenderWidget::SetCursorLocked(bool locked, bool follow_aspect_ratio)
{
  ...
  if (follow_aspect_ratio)
  {
    ...
    {
      float new_half_width = float(render_rect.width()) / (aspect_ratio.x * 2.f);
      // Only ceil if it was >= 0.25
      float ceiled_new_half_width = std::ceil(std::round(new_half_width * 2.f) / 2.f);
      int x_center = render_rect.center().x();
    ...
    {
      float new_half_height = render_rect.height() / (aspect_ratio.y * 2.f);
      float ceiled_new_half_height = std::ceil(std::round(new_half_height * 2.f) / 2.f);
      int y_center = render_rect.center().y();
