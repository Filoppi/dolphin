...
public:
  ...
  // If the edit condition isn't simplfied, its value might change at runtime, but this
  ...
  void SimplifyIfPossible() override
  {
    ...
    // Strip spaces and line breaks because the expression keeps them.
    std::string stripped_expression(StripSpaces(m_value.m_input.GetExpression()));
    ValueType value;
