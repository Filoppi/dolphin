...
public:
  ...
  // The control (input/output) gate is per thread per channel, this is becaue
  // every input channel needs it's own focus history flags, and because
  ...
  // consistency. This also sets the current gate channel to the specified one.
